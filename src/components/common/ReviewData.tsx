            ...
            <div className="max-w-[1272px] px-4 mx-auto">
                ...
                <div className="flex flex-wrap max-w-[1240px] w-full pt-7 lg:gap-x-[1.62%] max-lg:gap-5 gap-y-5">
                    {sortedReviews.map((item, index) => (
                        <div key={index} className="w-[49.19%] max-lg:w-full border border-black/10 rounded-[20px] py-7 px-8 max-sm:p-6 max-lg:px-7">
                            <div className="flex w-full justify-between items-center">
                                <span>{ }</span>
                                <span className="cursor-pointer flex"><DotsIcon /></span>
                            </div>
                            <div className="flex gap-1 items-center pt-[15px] max-md:pt-3">
                                <p className="font-bold text-xl max-md:text-base leading-[22%]">{item.userName}</p>
                                <GreenCheckedIcon />
                            </div>
                            <p className="pt-3 pb-6 max-md:pb-4 max-md:pt-2 text-black/60">{item.description}</p>
                            <p className="font-medium text-black/60 ">{item.uploadDate}</p>
                        </div>
                    ))}
                </div>
                <div className="flex justify-center items-center pt-[36.42px] max-md:pt-5">
                    <CustomButton buttonClass="w-[230px] h-[52px] max-md:w-[195px] max-md:h-[47px] text-base max-md:text-sm border-black/10 hover:bg-black hover:text-white" buttonText="Load More Reviews" />
                </div>
            </div>
        </div>
    );
