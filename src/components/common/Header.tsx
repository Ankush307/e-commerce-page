        ...
        <div className={`flex gap-6 items-center max-lg:fixed max-lg:top-0 max-lg:w-full max-lg:h-full duration-300 ease-linear max-lg:justify-center max-lg:flex-col max-lg:bg-black z-10 ${open ? "max-lg:left-0" : "max-lg:-left-full"}`}>
          {HEADER_LIST.map((item, index) => (
            <div key={index} className="relative group">
              <Link href={item.link} className="leading-[100%] whitespace-nowrap flex gap-1 items-center">{item.title} {item.submenu && <DropDownArrow />}</Link>
              {item.submenu && (
                  ...
                  <ul className="p-2">
                    {item.submenu.map((subItem, subIndex) => (
                      <li key={subIndex} className="py-1 px-4 hover:bg-gray-100" >
                        <Link className=" whitespace-nowrap" href={subItem.link} > {subItem.title}</Link>
                      </li>
                    ))}
                  </ul>
                </div>
