...
const ProductOverview = () => {
    ...
    }, [activeIndex, activeButton, quantity]);
        ...
        <div className="max-w-[1240px] mx-auto container pt-[60px] md:pt-20 max-xl:px-4">
            <div className="flex gap-10 max-[1025px]:flex-col max-lg:items-stretch max-xl:items-center">
                ...
                <div className="max-w-[600px] w-full flex flex-col">
                    <p className="font-integral-cf font-bold text-2xl md:text-custom-4xl mb-3.5 max-sm:mb-3 leading-[100%]">One Life Graphic T-shirt</p>
                    <div className="flex gap-4"><FourHalfStarIcon />
                    ...
                    <p className="text-black/60 mt-5 max-md:text-sm">This graphic t-shirt is perfect for any occasion. Crafted from a soft and breathable fabric, it offers superior comfort and style.</p>
                    ...
                    <div className="flex gap-4 mt-4">
                        {SELECT_COLOR.map((item, index) => (
                            <button key={index} className={`size-[38px] cursor-pointer flex justify-center items-center rounded-full ${item}`}
                                onClick={() => setActiveIndex(index)}> {index === activeIndex && <SelectIcon />}
                            </button>
                        ))}
                    </div>
                    <div className="max-w-[590px] w-full bg-black/10 h-[1px] my-6"></div>
                    ...
                    <div className="flex items-center justify-center gap-5">
                        ...
                        <button onClick={handleAddToCart} className="md:py-[15px] md:px-[140.5px] py-3.5 w-full hover:text-black hover:bg-transparent border border-transparent hover:border-black duration-300 font-satoshi-medium font-medium max-md:text-sm bg-black text-white rounded-full cursor-pointer">{buttonText}</button>
                    </div>
                </div>
            </div>
        </div>
