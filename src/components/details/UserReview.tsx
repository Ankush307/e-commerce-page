...
import ReviewsData from "../common/ReviewData";
...
const UserReview = () => {
    ...
    return (
        <div className="md:pt-20 pt-[50px]">
            <div className="max-w-[1240px] flex mx-auto relative">
                ...
                {DETAILS_TABS_LIST.map((item, index) => {
                    const itemSlug = item.toLocaleLowerCase().replaceAll(" ", "-").replace("&", "and");
            ...
            {tab === "rating-and-reviews" ? (
                <ReviewsData />) : (
                <div className="mt-4"><h2 className="text-lg text-center font-bold">Comming soon</h2></div>)}
        </div>
    );
};
